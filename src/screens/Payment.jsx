...
export default function Payment() {
  ...
  const artisanName = params.get("artisanName") || "Musa Ibrahim";
  const bookingId = params.get("bookingId");
  const [method, setMethod] = useState("card");

  const goToSuccess = () => navigate(`/payment-success?artisanName=${encodeURIComponent(artisanName)}`);
      ...
      {METHODS.map((m) => (
        <button
          ...
          className={`flex items-center justify-between border rounded-2xl px-4 h-[73px] transition ${
            method === m.id ? "border-[#1C4CD1]" : "border-[#E5E7EB]"
          }`}
        >
          ...
          <span className={`size-5 rounded-full border-2 ${method === m.id ? "border-[#1C4CD1] bg-[#1C4CD1]" : "border-[#E5E7EB]"}`} />
        </button>
      ))}
    </div>
  );
  ...
  const Summary = () => (
    <div className="border border-[#E5E7EB] rounded-2xl p-4 flex flex-col gap-3">
      ...
      <Button onClick={goToSuccess}>💰 Pay ₦19,163 &amp; Release Funds</Button>
    </div>
  );
      ...
      <div className="md:hidden flex flex-col h-full w-full">
        ...
        <div className="flex-1 overflow-y-auto px-6 pt-2 flex flex-col gap-5 pb-4">
          ...
          <div className="border border-[#E5E7EB] rounded-2xl p-4 flex items-center gap-3">
            <Avatar size={48} />
            <div>
              <p className="text-[#1F2937] font-semibold">{artisanName}</p>
              <p className="text-[#6B7280] text-xs">Marked this job as done{bookingId ? ` · Booking #${bookingId}` : ""}</p>
            </div>
          </div>
          {MethodList()}
          {Summary()}
      ...
      <div className="hidden md:flex md:flex-col md:h-full md:w-full">
        ...
        <div className="flex-1 overflow-y-auto px-12 py-10">
          <div className="max-w-[1120px] mx-auto flex flex-col gap-2">
            <button onClick={() => navigate("/bookings")} className="text-[#6B7280] text-sm self-start mb-2">← Back to bookings</button>
            <h1 className="text-[#1F2937] text-3xl font-bold">Complete Payment</h1>
            <p className="text-[#6B7280] text-base mb-4">
              {artisanName} marked this job as done. Confirm and pay to release their funds.
            </p>
            <div className="flex gap-8 items-start">
              <div className="flex-1 flex flex-col gap-6">
                <div className="border border-[#E5E7EB] rounded-2xl p-5 flex items-center gap-4">
                  <Avatar size={80} />
                  <div>
                    <p className="text-[#1F2937] text-xl font-bold">{artisanName}</p>
                    <p className="text-[#6B7280] text-sm mt-1">Marked this job as done.</p>
                  </div>
                </div>
                {MethodList()}
              </div>
